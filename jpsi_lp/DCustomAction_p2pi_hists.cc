...
void DCustomAction_p2pi_hists::Initialize(JEventLoop* locEventLoop)
{
	...
	japp->RootWriteLock(); //ACQUIRE ROOT LOCK!!
	{
		...
		dProton_DeltaT = GetOrCreate_Histogram<TH1I>("Proton Delta T", "; t(Proton) - t(RF) at target (ns)", 100, -10, 10);
		dElectron_DeltaT = GetOrCreate_Histogram<TH1I>("Electron Delta T", "; t(Electron) - t(RF) at target (ns)", 100, -10, 10);
		dPositron_DeltaT = GetOrCreate_Histogram<TH1I>("Positron Delta T", "; t(Positron) - t(RF) at target (ns)", 100, -10, 10);
		...
		if(dMissingPID == Proton) {
		  ...
		  dM2pi = GetOrCreate_Histogram<TH1I>("M2pi", "; M_{#pi^{+}#pi^{-}}", 200, 2.0, 4.0);
...
bool DCustomAction_p2pi_hists::Perform_Action(JEventLoop* locEventLoop, const DParticleCombo* locParticleCombo)
{
	...
	auto locChargedHypo_Proton = static_cast<const DChargedTrackHypothesis*>(locParticles[0]);
	dProton_DeltaT->Fill( locChargedHypo_Proton->t0() - locEventRFBunch->dTime );
	auto locChargedHypo_Electron = static_cast<const DChargedTrackHypothesis*>(locParticles[1]);
	dElectron_DeltaT->Fill( locChargedHypo_Electron->t0() - locEventRFBunch->dTime );
	auto locChargedHypo_Positron = static_cast<const DChargedTrackHypothesis*>(locParticles[2]);
	dPositron_DeltaT->Fill( locChargedHypo_Positron->t0() - locEventRFBunch->dTime );
	...
	for(size_t loc_i = 0; loc_i < 3; ++loc_i) {
		if(locParticles[loc_i] == NULL) continue; // missing particle
                ...
                if(locParticles[loc_i]->PID() == Electron || locParticles[loc_i]->PID() == Positron)
